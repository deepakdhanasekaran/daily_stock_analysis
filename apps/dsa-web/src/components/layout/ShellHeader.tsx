...
export const ShellHeader: React.FC<ShellHeaderProps> = ({
  ...
  const localizedTitle = {
    '/': text.shell.home,
    '/chat': text.shell.chat,
    '/backtest': text.shell.backtest,
    '/settings': text.shell.settings,
  }[location.pathname] ?? { title: text.shell.defaultTitle, description: text.shell.defaultDescription };
    ...
    <header className="sticky top-0 z-30 border-b border-border/60 bg-background/84 backdrop-blur-xl">
      <div className="mx-auto flex h-16 w-full max-w-[1680px] items-center gap-3 px-4 sm:px-6 lg:px-8">
        ...
        <button
          ...
          aria-label={collapsed ? text.shell.expandSidebar : text.shell.collapseSidebar}
        >
          {collapsed ? <PanelLeftOpen className="h-5 w-5" /> : <PanelLeftClose className="h-5 w-5" />}
        </button>

        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-semibold text-foreground">{localizedTitle.title}</p>
          <p className="truncate text-xs text-secondary-text">{localizedTitle.description}</p>
        </div>
